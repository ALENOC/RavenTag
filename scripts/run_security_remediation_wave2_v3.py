...
# Duplicate change expression intentionally exists in two accounting branches.
...
# Accept the actual trailing-lambda parameter name (currently `msg`) instead of
# hard-coding `progress`, while preserving that parameter in the replacement.
...
# Before the base script writes MainActivity, extend its generated transformation
# so the result string carries the exact recipient amount. This keeps MAX UI/history
# accounting honest and does not alter ordinary-send semantics.
needle = "write(main,s)\n\n# ---------------------------------------------------------------------------\n# Tests:"
extra = r'''# Exact amount actually sent is returned for UI/history accounting.
...
# MainViewModel parses the exact sent amount and uses it for display, optimistic
# balance accounting and tx history. Normal sends still equal the user's amount.
s = read(main)
